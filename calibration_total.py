...
class Calibration:
    ...
    def __init__(self, storage, pdict, location, pop_location, start_day, end_day,
                 ...
                 school_days=None, school_changes=None, to_plot = ['new_diagnoses']):
        '''
        Args:
    ...
    def create_sim(self, x, verbose=0):
        ...
        if len(x) == 5:
            ...
            # Create the sim
            sim = cv.Sim(pars, datafile=self.datafile, use_waning=True)
    ...
    def run_msim(self):
        ...
        self.sim = sim
        self.mismatch = sim.fit.mismatch
    ...
    def objective(self, x):
        ...
        #print('x in op_objective function', x)
        self.create_sim(x=x)
        self.run_msim()
    ...
    def worker(self):
        ...
        study = op.load_study(storage=self.storage, study_name=self.name)#, sampler=op.samplers.CmaEsSampler())
    ...
    def make_study(self):
        ...
        return op.create_study(storage=self.storage, study_name=self.name)#, sampler=op.samplers.CmaEsSampler())


    def load_study(self):
        return op.load_study(storage=self.storage, study_name=self.name)#, sampler=op.samplers.CmaEsSampler())
...
def model(cal):
    ...
    if do_plot:
        ...
        sim.run()
        sim.plot(to_plot=cal.to_plot, fig_args={'figsize': figsize})
    ...
    if do_plot:
        ...
        sim.run()
        sim.plot(to_plot=cal.to_plot, fig_args={'figsize': figsize}, do_save=True)
